...
不需要任何基础，打开 `notebooks/00_basic/11_quickwin_cartpole.ipynb`（彩蛋课），
按三次 `Shift+Enter`，你会看到一个训练好的 AI 把倒立摆稳满 500 分。
**它就是你 10 周后要亲手做出的东西。** 以后在语法枯燥期、调参崩溃期怀疑人生时，
回来再按一次。

### 2. 里程碑地图：每一阶段你都能「做出点什么」

这门课的设计原则是**每周都有看得见的产物**，不是攒到最后才见效：

| 时间点 | 你将亲手做出 | 成就感来源 |
|--------|-------------|-----------|
| 第 1 周末（B-2） | 自己的 Git 仓库 + 第一次提交/推送 | 「我的代码上云了」 |
| 第 2 周（B00/B-1） | 在终端里跑起第一个 Python 程序 | 黑窗口不再可怕 |
| 第 4–5 周（B03/B05） | 让弹簧、单摆在屏幕上动起来 | 公式变成了动画 |
| 第 8 周（B07/B08） | PID 调参 Web 面板 + 仿真工作台 | 第一个「作品」 |
| W04 | 你训练的 DQN 从瞎玩到稳住倒立摆 | 第一条亲手画的学习曲线 |
| W09 | 让专家 AI 当老师，教出「学生」AI | 理解模仿学习 |
| W11 | 自定义无人机悬停策略 | 阶段一毕业作品 |
| W28 | Gazebo 里仿真无人机自主导航 | 算法接进了机器人系统 |
| W38 | Capstone 毕业项目 + 演示视频 | 简历上的一行 |

### 3. 低谷预警：这些地方想放弃是正常的

| 时间点 | 典型症状 | 应对 |
|--------|----------|------|
| B01–B03（语法期） | 「背语法有什么用」 | 练习全是信号/电路小仿真，**别跳练习**；跑一遍彩蛋课回血 |
| W01（数学墙） | Bellman 方程看不懂 | 只需要会算 3 状态的小例子；回自检清单第 5 题的资源补 2 小时 |
| W05–W08（调参期） | 训练不收敛，怀疑机器坏了 | 不收敛是常态不是事故；按 `journal/troubleshooting.md` 记录现象→改一个变量→再跑 |
| 阶段二/三开头 | 环境安装报错连环 | 严格按 `docs/phaseN_*.md`；卡住先记录完整报错，按 B-1 的模板提问 |

**每周仪式**（详见文末「学习方法约定」）：周末 200 字 journal 小结 +
把本周最大的报错记入错题本。坚持 4 周后回看，进步是肉眼可见的——
这比任何鸡汤都更能维持信心。
...
| 彩蛋 | 先看终点：训练好的 AI 平衡倒立摆 | `11_quickwin_cartpole.ipynb` | 无需基础，装完环境即可运行；信心补给站 |
